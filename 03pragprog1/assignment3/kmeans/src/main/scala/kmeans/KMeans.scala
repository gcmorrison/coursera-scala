...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.isEmpty) means.map(p => p -> GenSeq[Point]()).toMap else points groupBy (p => findClosest(p, means))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    ...
    var idx = 0
    while (hasConverged && idx < oldMeans.size) {
      hasConverged = oldMeans(idx).squareDistance(newMeans(idx)) <= eta
      idx += 1
    }
    hasConverged
  }
