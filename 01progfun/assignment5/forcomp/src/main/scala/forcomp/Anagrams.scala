...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    ((w groupBy ((c: Char) => c.toLower)) map { case (char: Char, letters: String) => (char, letters.length) }).toList.sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences(s.foldLeft("")((word: Word, acc: Word) => word + acc))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(Nil)
    case head :: tail => {
      def expand(occ: (Char, Int)) = (for (count <- 0 to occ._2) yield (occ._1, count)).toList
      (for {
        occ <- expand(head)
        combined <- combinations(tail)
      } yield combined match {
        case Nil => List(occ)
        case x => occ :: x
      }) map (f => f filter (p => p._2 > 0))
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.foldLeft[Occurrences](Nil)((acc, current) => {
      val yEntry = y.filter(entry => current._1 == entry._1)
      if (yEntry.isEmpty) current :: acc else (yEntry.head._1, current._2 - yEntry.head._2) :: acc
    }).filter(p => p._2 > 0).sorted
  }
